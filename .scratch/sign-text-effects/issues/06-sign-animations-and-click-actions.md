...
**What to build:** Add the later-tier animated effects, with animation itself
available to everyone by default but off until enabled by policy. The animator
needs a tick loop, tracked-chunk gating, a configured budget, and frames that
never persist: saving a sign re-renders its base frame from stored source. Add
admin-only `<cmd:...>`, `show_dialog`, and `custom` click tags for waxed signs,
with an explicit security boundary around server-side actions. This admin-only,
off-by-default click policy is a security exception, not a balance rule.
...
- [ ] Animation is available to everyone by default but remains disabled until
      the configured policy enables it and the budget permits it
...
- [ ] `<cmd:...>` is admin-only and off by default as a security exception,
      producing a `run_command` click action only on waxed signs
- [ ] `show_dialog` and `custom` click tags are admin-only, preserve their
      vanilla dispatch restrictions, are off by default, and are available only
      on waxed signs
- [ ] No `open_url` server-side action is fabricated or dispatched by the
      feature
- [ ] Click-action parsing and execution are framed as a security boundary, not
      a balance rule: non-admin authors cannot write server commands, dialogs,
      or custom actions
