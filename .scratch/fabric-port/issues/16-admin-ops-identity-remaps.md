...
- **Properties note:** the swapped profile carries no property map (skin textures), matching Portal reality — the Portal's offline-mode backends never saw the real account's textures either, and a signed texture set would not validate against the aliased UUID anyway. The two aliased players render with default skins; not a deviation, but worth knowing at cutover.
