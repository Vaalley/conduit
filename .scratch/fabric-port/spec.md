...
**Inbound chat for the aliased identities** (ticket 22)

58. The two aliased players **receive** all player-voiced chat as **system** messages, decorated server-side with the same chat type — the inbound mirror of entry 57. A vanilla 26.2 client gates player-voiced chat, and only player-voiced chat, behind client-account state the server can neither see nor influence (the friends-only restriction, the profile/options chat restrictions, commands-only visibility), and in production the two aliased players landed on the wrong side of one of those gates: they could send, and saw system traffic (private messages, presence lines), but no one else's chat. The system channel passes every such gate, so `ServerPlayer.sendChatMessage` is redirected onto it for aliased recipients only — everyone else keeps signed, reportable player chat untouched. The cost mirrors entry 57's blast radius on the inbound side: lines these two are *shown* carry no signature on their client, so they cannot chat-report them there; the server log and everyone else's copies stay intact.
