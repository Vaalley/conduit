...
class MergeRegions(
     ...
     * stage the result only if there was something to write.
     *
    ...
    fun sweep(): MergeRegionsReport {
        ...
        // copy the merge commits. It doubles as the thing the overlap check is
        // asked of.
        ...
        if (rewrite) stage(RegionStore.serialize(swept))
    ...
    /**
     * The swept file built under the staging directory and **left there**. This
     * sweep never commits: `regions.json` goes into the live save with everything
     * else the merge built, in [MergeStaging]'s one move, so a later phase that
     * fails cannot leave rewritten Regions standing beside chunks that never
     * arrived (merge spec, User Story 45).
     *
     * It is staged as a replacement rather than an addition, because unlike the
     * relocated chunk data it lands on a file the merge read at the top of this
     * sweep — and a `regions.json` that has gone missing since is a save that
     * changed underneath the merge.
     *
    ...
    private fun stage(text: String) {
        Files.writeString(staging.replacing(regionsFile), text)
    }
