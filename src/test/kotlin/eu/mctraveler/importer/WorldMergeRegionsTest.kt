 ...
 *
 * Driven through the merge command itself against a synthetic run directory, so
 * what is asserted is the `regions.json` a server would boot from rather than
 * the shape of any intermediate step. [MergedDeploymentFixture]'s geography puts
 * the landmass at x +8192, z +0 — an eighth of that in the nether — and every
 * coordinate below is arrived at by hand from those two numbers.
 */
class WorldMergeRegionsTest {
    ...
    private fun merge() = WorldMerge(save.plan()).run()
    ...
    fun `the placement still leads the report, because it is what the operator has to accept`() {
        ...
        assertEquals(report.placement.lines() + report.regions.lines(), report.lines())
