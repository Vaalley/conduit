package eu.mctraveler.smoke

import eu.mctraveler.bootstrap.ConduitBootstrap
import eu.mctraveler.bootstrap.RuntimeHost
...
object SmokeHook : DedicatedServerModInitializer {
    ...
    override fun onInitializeServer() {
        ...
        ServerLifecycleEvents.SERVER_STARTED.register { server ->
            // This entrypoint must not link runtime classes: on the smoke server
            // the runtime lives in the bootstrap's side-loaded classloader, so
            // the bootstrap's own signal is the one to check — and it proves the
            // whole side-loading path (jar discovered, service found, started).
            check(RuntimeHost.isRuntimeActive()) {
                "smoke: server started but the MCTraveler runtime is not active"
            }
    ...
    private fun checkDimensions(server: MinecraftServer) {
        for (dimension in listOf(Level.OVERWORLD, Level.NETHER, Level.END, modDimension("embassies"))) {
    ...
    private fun modDimension(path: String): ResourceKey<Level> =
        ResourceKey.create(Registries.DIMENSION, Identifier.fromNamespaceAndPath(ConduitBootstrap.MOD_ID, path))
}
