...
object ChatFeature {
    ...
    fun register() {
        ServerMessageEvents.ALLOW_CHAT_MESSAGE.reloadable.register(::reformatPlayerChat)
        ServerMessageEvents.ALLOW_GAME_MESSAGE.reloadable.register { _, message, _ ->
        ...
        // Players online before a hot reload were already announced by the previous
        // runtime, so they must get a leave line later without a fresh join line.
        Conduit.onHotActivate { server ->
            server.playerList.players.forEach { announced += it.uuid }
        }
    }
